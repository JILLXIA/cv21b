...
def fit_one_epoch(net, yolo_loss, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, cuda):
    ...
    torch.save(model.state_dict(), 'logs/NNewEpoch%d-Total_Loss%.4f-Val_Loss%.4f.pth'%((epoch + 1), total_loss / (epoch_size + 1), val_loss / (epoch_size_val + 1)))
...
if __name__ == "__main__":
    ...
    #------------------------------------------------------#
    num_classes = load_data.classDicSize()
    #num_classes = 471
    #----------------------------------------------------#
    #   先验框anchor的路径
    #----------------------------------------------------#
    ...
    #   权值文件请看README，百度网盘下载
    #------------------------------------------------------#
    model_path      = "logs/NNewEpoch31-Total_Loss215.4936-Val_Loss218.5317.pth"
